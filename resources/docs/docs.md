...
The `home-routes` handler in the `agile-sliders.routes.home` namespace
...
The `home-page` function will in turn call the `agile-sliders.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `agile-sliders.handler` namespace:
...
Request middleware functions are located under the `agile-sliders.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `agile-sliders.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `agile-sliders.core` in some other namespace.
